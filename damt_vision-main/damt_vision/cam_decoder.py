#!/usr/bin/env python3
import rclpy, math, cv2, os, random, numpy as np
...
class CameraDecoder(Node):
    def __init__(self):
        ...
        #subs und pubs
        self.image_sub = self.create_subscription(CompressedImage, '/camera/image/compressed', self.image_callback, 5)
    ...
    #je nach dem ob auf Rechner oder Roboter gestartet
    ...
    def image_callback(self, msg):
        grey = cv2.cvtColor(cv2.imdecode(np.frombuffer(msg.data, np.uint8), cv2.IMREAD_COLOR), cv2.COLOR_BGR2GRAY) 
        h, self.breite = grey.shape
        offset = 400 #müsste dynamisch der bildgröße angepasst werden
        
        #beschneidung der Bereiche nach Mapgröße und Linienbreite anpassen
        ...
        #grey = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) #ganzes graustufen Bild zum Debuggen
        blur_geradeaus = cv2.GaussianBlur(geradeaus, (5, 5), 0) #unschärfen (x,y) muss ungerade sein
        ...
        #_, thresh = cv2.threshold(grey.copy(), binary_treshold, 255, cv2.THRESH_BINARY) #Debug ansicht
        ...
        #Anzeigeoptionen zum Debuggen
        if not self.is_headless():
            #cv2.drawContours(grey, self.contours_geradeaus, -1, (0, 255, 0), 2)
            #cv2.drawContours(grey, (contours_rechts + contours_links), -1, (0, 0, 255), 2)

            #cv2.namedWindow("native image", cv2.WINDOW_NORMAL)
            #cv2.imshow('native image', image)
            ...
            # cv2.namedWindow("thres image", cv2.WINDOW_NORMAL)
            # cv2.imshow('thres image', thresh)

            #cv2.namedWindow("contourimage", cv2.WINDOW_NORMAL)
            #cv2.imshow('contourimage', image)

            # cv2.namedWindow("fahr image", cv2.WINDOW_NORMAL)
            # cv2.imshow('fahr image', geradeaus)

            # cv2.namedWindow("rechts", cv2.WINDOW_NORMAL)
            # cv2.imshow('rechts', rechts)

            # cv2.namedWindow("links", cv2.WINDOW_NORMAL)
            # cv2.imshow('links', links)

            #cv2.namedWindow("alt_image", cv2.WINDOW_NORMAL)
            #cv2.imshow('alt_image', altImage)
        ...
        if self.State == "fahren":##################################################################### STATEMACHINE: Fahren
            if self.controll == "umkehren":#manuelle Steuerung kann jederzeit umkehren, TODO man kann allerdings so in ein Errorstate kommen
                self.State = "abbiegen"
            if self.kreuzung_rechts or self.kreuzung_links:########################################### übergang zu abbiegen
            ...
            elif np.mean(self.error_queue) >= 500:#################################################### Fall: überschwenkt #TODO könnte noch optimiert werden, aber funktioniert einigermaßen
            ...
            pass    # deaktiviert die Zustandsüberänge, so lange wie der Roboter braucht um die aktuelle Aktion zu beenden
        ...
        if (self.get_clock().now() - self.timer_start).nanoseconds < 1500000000: #Phase 1 Deg-Kontrolle
            ...
            if self.richtung == "rechts":
                if -diff > 80.0:
                    ...
                    twist.angular.z = 0.0
                #twist.angular.z = -0.65 #ermittelte Richtwerte. Könnte je Roboter variieren. die Diff zwischen Beiden Motoren wird einfach Linear berachtet
    ...
    def line_callback(self):
        ...
        if self.get_clock().now() - self.timer_start < self.timer_dauer:
            if self.mode == False:### er sieht die Linie, muss nur nochmal feinjustieren
                ...
                if (self.get_clock().now() - self.timer_start).nanoseconds < self.timer_dauer.nanoseconds / 2: # erste Hälfte der Zeit
                    twist.angular.z = 1.0
